...
class History {
  constructor ($eventBus, $serviceLocator, $logger, $window) {
    ...
    this._logger = $logger;
    this._history = [];
    this._clear = JSON.parse($window.localStorage.getItem('DEBUGGER_CLEAR_CONSOLE'));
    this._tracking = JSON.parse($window.localStorage.getItem('DEBUGGER_TRACKING'));

    if (JSON.parse($window.localStorage.getItem('DEBUGGER_ENABLED'))) {
